...
async def start(event):
  await event.reply("**🌀GrupTaggerBot**\n **İle Grubunuzdakı Nerdeyse Tüm Üyelere Etiket Ata bilirim \nKomutlar için =======> /help yazın**",
		      ...
		      [Button.url('Developer👨🏻‍💻', 'https://t.me/GERGINBIOM')],
                    ),
                    link_preview=False
                   )
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      if event.chat_id not in anlik_calisan:
        await event.respond("Işlem Başarıyla Durduruldu\n\n**Buda sizin reklamınız ola bilir @HAYATAADIMATMA**❌")
        return
      if usrnum == 5:
        await client.send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
        await asyncio.sleep(2)
...
async def mentionall(event):
  ...
  if mode == "text_on_reply":
    ...
    async for usr in client.iter_participants(event.chat_id):
      ...
      usrtxt += f"[{random.choice(stag)}](tg://user?id={usr.id}) "
      if event.chat_id not in anlik_calisan:
        await event.respond("Işlem Başarıyla Durduruldu\n\n**Buda sizin reklamınız ola bilir @GerginUserBot**❌")
        return
      if usrnum == 1:
        await client.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
...
async def handler(event):
    ...
    await event.reply('__Botun Reklam Menüsü__\n**Reklam Veya İş Birliğimi Yapmak İstiyorsunuz Botun** [Sahibi](https://t.me/GERGINBIOM) **İle İletişime Geçiniz**')

print(">> Bot çalıyor merak etme 🚀 @GERGINBIOM bilgi alabilirsin <<")
